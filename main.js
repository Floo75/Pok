...
(() => {
  ...
  const caughtBar = document.getElementById('caughtBar');
  // (Réglages supprimés)
  // Intro de niveau
  ...
  // appliquer immédiatement et écouter les changements
  resizeCanvas();
  ...
  // Barème d'objectifs selon le niveau
  function getLevelObjectives(level){
    const lvl = Math.max(1, Math.min(40, level));
    const common = 10 + 5 * (lvl - 1); // 10,15,20,...
    const rare = Math.round(common / 2); // ratio ~2:1
    return { common, rare };
  }
  ...
  function renderLevelIntro() {
    ...
    levelIntroContent.innerHTML = `
      <p>Temps: <strong>${timeStr}</strong> • Objectif: <strong>${targetCommon} communs</strong> ou <strong>${targetRare} rares</strong></p>
    ...
    `;
    // Mettre à jour la ligne du tutoriel concernant la durée de vie
    try {
  ...
  function onLevelSuccess() {
    ...
    if (levelResultText) {
      levelResultText.innerHTML = `Temps: <strong>${timeStr}</strong><br>` +
        `Commun: <strong>${capturedCommon}</strong> • Rares: <strong>${capturedRare}</strong> • Total: <strong>${totalCaptures}</strong><br>` +
  ...
  let running = true;
  let isPaused = false;
  // Responsive: ajuster canvas en fonction de la taille CSS et du devicePixelRatio
  function resizeCanvas() {
    ...
    if (canvas.width !== newW || canvas.height !== newH) {
      canvas.width = newW;
      canvas.height = newH;
      WORLD.w = newW; WORLD.h = newH;
      // Arène plein écran
      arenaBounds = { x: 0, y: 0, w: WORLD.w, h: WORLD.h };
      // reclamp joueur
      const margin = 16;
      player.x = clamp(player.x, arenaBounds.x + margin, arenaBounds.x + arenaBounds.w - margin);
      player.y = clamp(player.y, arenaBounds.y + margin, arenaBounds.y + arenaBounds.h - margin);
    }
  }
  ...
  // Touch: suivre le doigt (premier touch)
  ...
  canvas.addEventListener('touchstart', (e) => { e.preventDefault(); setTouchTarget(e); }, { passive: false });
  canvas.addEventListener('touchmove', (e) => { e.preventDefault(); setTouchTarget(e); }, { passive: false });
  ...
  (function initItemSprites(){
    ...
    t.onload = () => { trapSprite = t; };
    t.onerror = () => {
      // deuxième tentative sur un autre chemin
      const t2 = new Image();
      t2.crossOrigin = 'anonymous';
      t2.onload = () => { trapSprite = t2; };
      t2.onerror = () => {
        // Fallback data URL SVG (bombe stylisée)
        const svg = `data:image/svg+xml;utf8,${encodeURIComponent("<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'><circle cx='28' cy='36' r='18' fill='#37474f'/><rect x='38' y='16' width='10' height='8' rx='2' fill='#546e7a'/><path d='M48 12 l6 -6' stroke='#fdd835' stroke-width='3' stroke-linecap='round'/><path d='M52 10 l6 -6' stroke='#ff7043' stroke-width='3' stroke-linecap='round'/></svg>")}`;
        const img2 = new Image(); img2.src = svg; trapSprite = img2;
      };
      t2.src = 'assets/sprites/bomb.jpg';
    };
    // Essayer d'abord le chemin 'sprites/' demandé par l'utilisateur
    t.src = 'sprites/bomb.jpg';

    const m = new Image();
    m.crossOrigin = 'anonymous';
    m.onload = () => { medSprite = m; };
    m.onerror = () => {
      const m2 = new Image();
      m2.crossOrigin = 'anonymous';
      m2.onload = () => { medSprite = m2; };
      m2.onerror = () => {
        // Fallback data URL SVG (trousse de secours)
        const svg = `data:image/svg+xml;utf8,${encodeURIComponent("<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'><rect x='8' y='18' width='48' height='32' rx='6' fill='#e53935'/><rect x='28' y='22' width='8' height='24' fill='white'/><rect x='20' y='30' width='24' height='8' fill='white'/><rect x='20' y='12' width='24' height='8' rx='3' fill='#c62828'/></svg>")}`;
        const img2 = new Image(); img2.src = svg; medSprite = img2;
      };
      m2.src = 'assets/sprites/first_aid.png';
    };
    m.src = 'sprites/first_aid.png';
  })();
  ...
  function captureCreature(index) {
    ...
    // suivi commun/rare pour succès de niveau
    if (RARE_TYPES.has(c.typeKey)) capturedRare += 1; else capturedCommon += 1;
    updateObjectives();
    if (capturedCommon >= targetCommon || capturedRare >= targetRare) {
      onLevelSuccess();
    }
  ...
  function renderCollection() {
    ...
    // Rendu barre horizontale en haut
    if (caughtBar) {
      const topItems = items.slice().sort((a,b)=> b.count - a.count);
      const html = topItems.map((it) => {
        const src = it.imgSrc || '';
        return `<span class="caught"><img src="${src}" alt="${it.name}"><span class="n">×${it.count}</span></span>`;
      }).join('');
      caughtBar.innerHTML = html;
    }
  }
  ...
  function restartGame() {
    ...
    accelerated = false;
    capturedCommon = 0; capturedRare = 0;
    updateLevelPill();
  ...
  function updateObjectives() {
    const commonsLeft = Math.max(0, targetCommon - capturedCommon);
    const raresLeft = Math.max(0, targetRare - capturedRare);
    if (objectivesHUD) {
      objectivesHUD.textContent = `Communs restants: ${commonsLeft} | Rares restants: ${raresLeft}`;
    }
  }
  ...
  // Halo de suivi souris/doigt
  function drawPointerHalo() {
    if (targetMouseX == null || targetMouseY == null) return;
    ctx.save();
    const x = targetMouseX, y = targetMouseY;
    const r = 22;
  ...
  function renderScores() {
    ...
    if (arr.length === 0) {
      scoresList.innerHTML = '<p>Aucun score enregistré pour le moment.</p>';
      return;
    }
    const rows = arr.map((e) => {
      ...
      return `<div style="border:2px solid #eef3f9;border-radius:10px;padding:8px;margin:6px 0;background:#fbfdff">
        <strong>${new Date(e.ts).toLocaleString()}</strong><br>
        Niveau ${e.level} • Arène: ${e.arena} • Temps: ${t}<br>
        Score: <strong>${e.score}</strong> • Communs: ${e.commons} • Rares: ${e.rares} • Total: ${e.total}
      </div>`;
    }).join('');
  ...
  function showScores() {
    if (!scoresOverlay) return;
    renderScores();
    // éviter que le menu bloque les clics sur l'overlay scores
    if (menuOverlay && !menuOverlay.classList.contains('hidden')) {
      wasInMenuWhenOpeningScores = true;
      menuOverlay.classList.add('hidden');
    } else {
      wasInMenuWhenOpeningScores = false;
    }
  ...
  function hideScores() {
    ...
    if (wasInMenuWhenOpeningScores && menuOverlay) {
